...
class NominaSpec extends  Specification{
    ...
    def 'Generar un complemento valido'(){
        ...
        Nomina nomina = buildComplemento()

        and: 'Un StringWriter'
        StringWriter writer = new StringWriter()

        when: 'Transformamos a xml el complemento'
        NominaUtils.getMarshaller().marshal(nomina,writer)
    ...
    def buildComplemento(){
        ...
        nomina.fechaPago = NominaUtils.toDate(new Date())
        nomina.fechaInicialPago = NominaUtils.toDate(new Date())
        nomina.fechaFinalPago = NominaUtils.toDate(new Date())

        Nomina.Receptor receptor = factory.createNominaReceptor()
        receptor.numEmpleado = '10'
        receptor.curp = 'CUAR750525MDFRMS02'
        nomina.receptor = receptor
        nomina.receptor.tipoContrato = '99'
        nomina.receptor.tipoRegimen = '09'
        nomina.receptor.periodicidadPago = '99'
        nomina.receptor.claveEntFed = CEstado.DIF
        return nomina
    }
